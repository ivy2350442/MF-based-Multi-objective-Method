...
def main():
    ...
    for test in range(cv):
        ...
        #knn
        s_knn = LogisticRegression(C = 7.6)
        t_knn = LogisticRegression(C = 7.0)
        r_knn = LogisticRegression(C = 7.6)
        ent_knn = LogisticRegression(C = 9.8)
        g_knn = LogisticRegression(C = 1.4)
        e_knn = LogisticRegression(C = 10.0)
        ...
        if test == 2:
            ...
            plt.savefig("LOG.pdf", format='pdf', dpi=1000)
            plt.show()
